...
//    typedef  std::pair<Vector3f , *> Record;
    ...
    std::vector<Photon *> kNN(const Vector3f & pos,int k, std::vector<Photon> & elements) const ;
...
void KdTree :: build(  std::vector<Photon> & photons)  {
     ...
     std:: function<KdNode *(std::vector<Photon> &,int ,int  ) > solve= [&](std::vector<Photon> &  elements,int l,int r) -> KdNode  *
        {
                 if (r==l)
                     return nullptr;
                 ...
                 BoundingBox3d box;
                 for(const Photon& photon : elements){
                 ...
                 node->left = solve(elements,l,mid);
                 node->right = solve(elements,mid+1,r);

                 return node;
             };
    root= solve(photons,0,photons.size());

}



std::vector<Photon *> KdTree::kNN(const Vector3f & pos,int k, std::vector<Photon> & elements)const{
    ...
    std::function<void(const  KdNode *)>  solve =[&] (const  KdNode * node){
            ...
            {
            auto  mx= std::max(abs(bbox.min.x()-pos.x()),abs(bbox.max.x()-pos.x()) );
            auto  my=std:: max(abs(bbox.min.y()-pos.y()),abs(bbox.max.y()-pos.y()) );
            auto  mz= std::max(abs(bbox.min.z()-pos.z()),abs(bbox.max.z()-pos.z()) );
            Vector3f d(mx,my,mz);
